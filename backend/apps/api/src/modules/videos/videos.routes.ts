...
export async function videoRoutes(fastify: FastifyInstance) {
    ...
    server.post(
        ...
        async (request, reply) => {
            ...
            const { user, limits } = await videoService.validateVideoUpload(userId, durationSeconds, width, height);
            ...
            logger.info({ videoId: video.id, jobId: job.id, resolution: `${width}x${height}` }, 'Video upload initiated');
